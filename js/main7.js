...
const winner = function (board2D) {
  ...
  for (let i = 0; i < n; i++) {
    let columns = [];
    // check row win
    if ( board2D[i].join('') === previousPlayer.repeat(n) ){
      console.log(`WIN FOR ${previousPlayer}!!!!`);
      return;
    }
    ...
    // console.log(columns);
    // console.log(previousPlayer.repeat(n));
    if (columns.join('') === previousPlayer.repeat(n)) {
      console.log(`WIN FOR ${previousPlayer}!!!!`);
      return;
    }
    // console.log(columns);
  ...
  // check diag win
  if (diag.join('') === previousPlayer.repeat(n)) {
    console.log(`WIN FOR ${previousPlayer}!!!!`);
    return;
  }

  //check anti-diagonal win
  if (antiDiag.join('') === previousPlayer.repeat(n)) {
    console.log(`WIN FOR ${previousPlayer}!!!!`);
    return;
  }
};
...
$(document).ready(function () {
  ...
  console.log('JQuery is ready - main7.js');
